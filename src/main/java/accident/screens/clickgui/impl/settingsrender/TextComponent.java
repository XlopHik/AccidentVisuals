...
public class TextComponent extends AbstractSettingComponent {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        Fonts.TEST.draw(textSetting.getName(), x + 3.7f, y + height / 2 - 4f, 6, applyAlpha(new Color(210, 210, 220, 200)).getRGB());
    ...
    private void renderTextContent(float boxX, float boxY, float deltaTime) {
        ...
        float fullTextWidth = Fonts.TEST.getWidth(displayText, 5);

        if (focused) {
            String beforeCursor = text.substring(0, cursorPosition);
            float cursorX = Fonts.TEST.getWidth(beforeCursor, 5);
        ...
        if (text.isEmpty() && !focused) {
            Fonts.TEST.draw("Enter text...", textAreaX, textY, 5, applyAlpha(new Color(100, 100, 105, 100)).getRGB());
        } else {
            if (focused && hasSelection() && selectionAnimation > 0.01f) {
                ...
                float selectionX = textAreaX + Fonts.TEST.getWidth(beforeSelection, 5) - textScrollOffset;
                float selectionWidth = Fonts.TEST.getWidth(selection, 5);
            ...
            Fonts.TEST.draw(displayText, textAreaX - textScrollOffset, textY, 5,
            ...
            if (focused && !hasSelection()) {
                ...
                if (cursorAlpha > 0.3f) {
                    String beforeCursor = text.substring(0, cursorPosition);
                    float cursorXPos = textAreaX + Fonts.TEST.getWidth(beforeCursor, 5) - textScrollOffset;
    ...
    private int getCursorIndexAt(double mouseX) {
        ...
        while (position < text.length()) {
            float currentWidth = Fonts.TEST.getWidth(text.substring(0, position + 1), 5);
