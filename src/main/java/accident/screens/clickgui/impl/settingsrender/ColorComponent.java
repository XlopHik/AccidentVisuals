...
public class ColorComponent extends AbstractSettingComponent {
    ...
    private float maxBottomY = Float.MAX_VALUE;

    public void setMaxBottomY(float maxBottomY) {
    ...
    public void setAlphaMultiplier(float alphaMultiplier) {
        super.setAlphaMultiplier(Math.max(0f, Math.min(1f, alphaMultiplier)));
    }
    ...
    private int getHexSelectionStart() { return Math.min(hexSelectionStart, hexSelectionEnd); }
    private int getHexSelectionEnd()   { return Math.max(hexSelectionStart, hexSelectionEnd); }
    ...
    private void clearHexSelection() { hexSelectionStart = -1; hexSelectionEnd = -1; }
    ...
    private void pasteHexFromClipboard() {
        ...
        if (clipboardText != null && !clipboardText.isEmpty()) {
            ...
            if (clipboardText.length() > remainingSpace) clipboardText = clipboardText.substring(0, remainingSpace);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        Fonts.TEST.draw(colorSetting.getName(), x + 4f, y + height / 2 - 4.5f, 6, applyAlpha(new Color(210, 210, 220, 200)).getRGB());
    ...
    private void renderColorPreview(int mouseX, int mouseY) {
        ...
        Color previewColor = new Color(getDisplayColor(), true);
    ...
    private void renderColorPicker(DrawContext context, int mouseX, int mouseY, float deltaTime) {
        ...
        float availableH = maxBottomY - pickerY;
        if (availableH <= 0) return;
        visibleHeight = Math.min(visibleHeight, availableH);
    ...
    private void renderSaturationBrightnessPalette(float paletteX, float paletteY, float paletteWidth, float paletteHeight, int mouseX, int mouseY) {
        ...
        Color handleColor = new Color(Color.HSBtoRGB(displayHue, displaySaturation, displayBrightness));
    ...
    private void renderHueSlider(float sliderX, float sliderY, float sliderWidth, float sliderHeight, int mouseX, int mouseY) {
        int[] hueColors = {
                Color.HSBtoRGB(0f, 1f, 1f),    Color.HSBtoRGB(1f/6f, 1f, 1f),
                Color.HSBtoRGB(2f/6f, 1f, 1f), Color.HSBtoRGB(3f/6f, 1f, 1f),
                Color.HSBtoRGB(4f/6f, 1f, 1f), Color.HSBtoRGB(5f/6f, 1f, 1f),
        ...
        Render2D.outline(sliderX, sliderY, sliderWidth, sliderHeight, 0.5f, new Color(100, 100, 105, hueOutlineAlpha).getRGB(), 3f);

        float handleY      = sliderY + displayHue * sliderHeight;
        float handleHeight = 3f + hueHandleAnimation;
        float handleWidth  = sliderWidth + 2f;

        int handleAlpha = clamp((int)(255 * expandAnimation * contentAlpha * alphaMultiplier));
        Render2D.rect(sliderX - 1, handleY - handleHeight / 2, handleWidth, handleHeight, new Color(255, 255, 255, handleAlpha).getRGB(), 1.5f);
        int handleShadowAlpha = clamp((int)(100 * expandAnimation * contentAlpha * alphaMultiplier));
        Render2D.outline(sliderX - 1, handleY - handleHeight / 2, handleWidth, handleHeight, 0.5f, new Color(0, 0, 0, handleShadowAlpha).getRGB(), 1.5f);
    }
    ...
    private void renderAlphaSlider(float sliderX, float sliderY, float sliderWidth, float sliderHeight, int mouseX, int mouseY) {
        ...
        int[] alphaGradient = {
                applyContentAlpha(new Color(baseColor, true), 0f).getRGB(),
                applyContentAlpha(new Color(baseColor, true), 0f).getRGB(),
        ...
        Render2D.outline(sliderX, sliderY, sliderWidth, sliderHeight, 0.5f, new Color(100, 100, 105, alphaOutlineAlpha).getRGB(), 3f);

        float handleY      = sliderY + displayAlpha * sliderHeight;
        float handleHeight = 3f + alphaHandleAnimation;
        float handleWidth  = sliderWidth + 2f;
    ...
    private void renderHexInput(float inputX, float inputY, float inputWidth, float inputHeight, int mouseX, int mouseY) {
        ...
        Color outlineColor = hexInputActive ? new Color(100, 140, 180, hexOutlineAlpha) : new Color(80, 80, 85, hexOutlineAlpha);
        ...
        float  labelWidth  = Fonts.TEST.getWidth(label, 5);
        int    labelAlpha  = clamp((int)(150 * expandAnimation * contentAlpha * alphaMultiplier));
        Fonts.TEST.draw(label, inputX + 4 + iconOffset, inputY + inputHeight / 2 - 2.5f, 5, new Color(140, 140, 150, labelAlpha).getRGB());
        ...
        Fonts.TEST.draw("#" + displayText, textStartX, textY, 5, new Color(210, 210, 220, textAlpha).getRGB());
        ...
        if (hexInputActive && !hasHexSelection()) {
            ...
            if (cursorAlpha > 0.3f) {
                ...
                float  cursorX       = textStartX + Fonts.TEST.getWidth(beforeCursor, 5);
                int    cursorAlphaInt = clamp((int)(255 * cursorAlpha * hexInputAnimation * expandAnimation * contentAlpha * alphaMultiplier));
                Render2D.rect(cursorX, inputY + 3, 0.5f, inputHeight - 6, new Color(180, 180, 185, cursorAlphaInt).getRGB(), 0f);
            }
        }
        ...
        Render2D.rect(miniPreviewX, miniPreviewY, miniPreviewSize, miniPreviewSize, new Color(150, 150, 150, miniCheckAlpha).getRGB(), 3f);
        Render2D.rect(miniPreviewX, miniPreviewY, miniPreviewSize, miniPreviewSize, applyContentAlpha(new Color(getDisplayColor(), true)).getRGB(), 3f);
        int miniOutlineAlpha = clamp((int)(80 * expandAnimation * contentAlpha * alphaMultiplier));
        Render2D.outline(miniPreviewX, miniPreviewY, miniPreviewSize, miniPreviewSize, 0.5f, new Color(80, 80, 85, miniOutlineAlpha).getRGB(), 3f);
    }
    ...
    private boolean isPaletteHover(double mouseX, double mouseY) {
        ...
        return mouseX >= paletteX && mouseX <= paletteX + paletteWidth && mouseY >= contentY && mouseY <= contentY + PALETTE_SIZE;
    }

    private boolean isHueSliderHover(double mouseX, double mouseY) {
        float contentX = x + SPACING;
        float contentY = y + height + SPACING + SPACING;
        return mouseX >= contentX && mouseX <= contentX + SLIDER_WIDTH && mouseY >= contentY && mouseY <= contentY + PALETTE_SIZE;
    }
    ...
    private boolean isAlphaSliderHover(double mouseX, double mouseY) {
        ...
        return mouseX >= alphaSliderX && mouseX <= alphaSliderX + SLIDER_WIDTH && mouseY >= contentY && mouseY <= contentY + PALETTE_SIZE;
    }

    private boolean isHexInputHover(double mouseX, double mouseY) {
        float contentX     = x + SPACING;
        float contentY     = y + height + SPACING + SPACING + PALETTE_SIZE + SPACING;
        float contentWidth = width - SPACING * 2;
        return mouseX >= contentX && mouseX <= contentX + contentWidth && mouseY >= contentY && mouseY <= contentY + 16;
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0) {
            if (isPreviewHover(mouseX, mouseY)) {
                expanded = !expanded;
                if (!expanded) { hexInputActive = false; draggingPalette = false; draggingHue = false; draggingAlpha = false; clearHexSelection(); }
                return true;
            }
            System.out.println("mouseClicked x=" + x + " y=" + y + " w=" + width + " expanded=" + expanded + " expandAnim=" + expandAnimation + " contentAlpha=" + contentAlpha + " mouse=" + mouseX + "," + mouseY);
            System.out.println("isPalette=" + isPaletteHover(mouseX, mouseY) + " isHue=" + isHueSliderHover(mouseX, mouseY) + " isAlpha=" + isAlphaSliderHover(mouseX, mouseY));
            if (expanded && expandAnimation > 0.8f && contentAlpha > 0.5f) {
                if (isPaletteHover(mouseX, mouseY)) { draggingPalette = true; updatePalette(mouseX, mouseY); hexInputActive = false; clearHexSelection(); return true; }
                if (isHueSliderHover(mouseX, mouseY)) { draggingHue = true; updateHue(mouseY); hexInputActive = false; clearHexSelection(); return true; }
                if (isAlphaSliderHover(mouseX, mouseY)) { draggingAlpha = true; updateAlpha(mouseY); hexInputActive = false; clearHexSelection(); return true; }
                if (isHexInputHover(mouseX, mouseY)) {
                    hexInputActive = true; hexInputText = getHexString(); hexCursorPosition = hexInputText.length();
                    hexSelectionStart = 0; hexSelectionEnd = hexInputText.length(); return true;
                } else if (hexInputActive) { applyHexInput(); hexInputActive = false; clearHexSelection(); }
            }
        }
        return false;
    }
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if (button == 0) {
            ...
            draggingPalette = false; draggingHue = false; draggingAlpha = false;
            if (wasDragging) { updateHexFromColor(); return true; }
        }
        return false;
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        switch (keyCode) {
            ...
            case GLFW.GLFW_KEY_BACKSPACE -> {
                if (hasHexSelection()) deleteHexSelectedText();
                else if (hexCursorPosition > 0) { hexInputText = hexInputText.substring(0, hexCursorPosition - 1) + hexInputText.substring(hexCursorPosition); hexCursorPosition--; }
                return true;
            }
            case GLFW.GLFW_KEY_DELETE -> {
                if (hasHexSelection()) deleteHexSelectedText();
                else if (hexCursorPosition < hexInputText.length()) hexInputText = hexInputText.substring(0, hexCursorPosition) + hexInputText.substring(hexCursorPosition + 1);
                return true;
            }
    ...
    private void updatePalette(double mouseX, double mouseY) {
        ...
        float paletteWidth = contentWidth - slidersWidth - SPACING;
        float paletteX     = contentX + slidersWidth + SPACING;
        colorSetting.setSaturation((float)((mouseX - paletteX) / paletteWidth));
        colorSetting.setBrightness(1f - (float)((mouseY - contentY) / PALETTE_SIZE));
    }

    private void updateHue(double mouseY) {
        float contentY = y + height + SPACING + SPACING;
        colorSetting.setHue((float)((mouseY - contentY) / PALETTE_SIZE));
    }

    private void updateAlpha(double mouseY) {
        float contentY = y + height + SPACING + SPACING;
        colorSetting.setAlpha((float)((mouseY - contentY) / PALETTE_SIZE));
    }
    ...
    private void applyHexInput() {
        ...
        try {
            int r, g, b, a = 255;
            if (hex.length() == 6) {
                r = Integer.parseInt(hex.substring(0, 2), 16); g = Integer.parseInt(hex.substring(2, 4), 16); b = Integer.parseInt(hex.substring(4, 6), 16);
            } else if (hex.length() == 8) {
                r = Integer.parseInt(hex.substring(0, 2), 16); g = Integer.parseInt(hex.substring(2, 4), 16); b = Integer.parseInt(hex.substring(4, 6), 16); a = Integer.parseInt(hex.substring(6, 8), 16);
            } else if (hex.length() == 3) {
                r = Integer.parseInt(hex.substring(0, 1) + hex.substring(0, 1), 16); g = Integer.parseInt(hex.substring(1, 2) + hex.substring(1, 2), 16); b = Integer.parseInt(hex.substring(2, 3) + hex.substring(2, 3), 16);
            } else { updateHexFromColor(); return; }
            float[] hsb = Color.RGBtoHSB(r, g, b, null);
            colorSetting.setHue(hsb[0]); colorSetting.setSaturation(hsb[1]); colorSetting.setBrightness(hsb[2]); colorSetting.setAlpha(a / 255f);
        } catch (NumberFormatException e) { updateHexFromColor(); }
    }
    ...
    public float getTotalHeight() {
        ...
        return height + totalExpandedHeight * expandAnimation;
    }

    public boolean isDragging() { return draggingPalette || draggingHue || draggingAlpha; }
}