...
public class TargetESP extends ModuleStructure implements IMinecraft {
    ...
    final SelectSetting mode = new SelectSetting("accident.module.targetesp.setting.mode.name", "accident.module.targetesp.setting.mode.desc")
            .value("Rhomb", "Ghost", "Chain", "Crystals", "Circle", "Inward", "Tornado", "Arrow")
            .selected("Rhomb");

    final SliderSettings crystalRotationSpeed = new SliderSettings("accident.module.targetesp.setting.crystalrotationspeed.name",
            "accident.module.targetesp.setting.crystalrotationspeed.desc")
            .range(0.1f, 2.0f)
            .visible(() -> mode.isSelected("Crystals"));

    final SliderSettings arrowSize = new SliderSettings("accident.module.targetesp.setting.arrowsize.name", "accident.module.targetesp.setting.arrowsize.desc")
            .range(0.3f, 2.0f)
            .visible(() -> mode.isSelected("Arrow"));

    final ColorSetting color1 = new ColorSetting("accident.module.targetesp.setting.color1.name", "accident.module.targetesp.setting.color1.desc")
            .setColor(new Color(255, 101, 57, 255).getRGB());

    final ColorSetting color2 = new ColorSetting("accident.module.targetesp.setting.color2.name", "accident.module.targetesp.setting.color2.desc")
            .setColor(new Color(255, 50, 150, 255).getRGB());

    final ColorSetting color3 = new ColorSetting("accident.module.targetesp.setting.color3.name", "accident.module.targetesp.setting.color3.desc")
            .setColor(new Color(150, 50, 255, 255).getRGB())
            .visible(() -> mode.isSelected("Ghost"));

    private Vec3d smoothedPos = null;
    private LivingEntity lastTarget = null;
    private LivingEntity delayedTarget = null;
    ...
    private float arrowIdleTimer   = 0f;
    private float arrowSpinTimer   = 0f;
    private float arrowSpinAngle   = 0f;
    private boolean arrowSpinning  = false;

    private static final float ARROW_IDLE_TIME = 3000f;
    private static final float ARROW_SPIN_TIME = 1200f;

    public TargetESP() {
        super("accident.module.targetesp.name", "accident.module.targetesp.desc", ModuleCategory.RENDER);
        instance = this;

        crystalRotationSpeed.setValue(0.5f);

        settings(mode, crystalRotationSpeed, arrowSize, color1, color2, color3);
    }
    ...
    public void onRender3D(WorldRenderEvent e) {
        ...
        hurtProgress = delayedTarget.hurtTime > 0 ? (float) delayedTarget.hurtTime / 10f : Math.max(0, hurtProgress - hurtDecay);

        Render3D.updateTargetEsp(deltaTime);

        if (mode.isSelected("Circle")) {
            renderCircle(e.getStack(), delayedTarget, alpha);
            return;
        }
        ...
        Vec3d targetPos = delayedTarget.getLerpedPos(partialTicks);

        if (lastTarget != delayedTarget || smoothedPos == null) {
            smoothedPos = targetPos;
            lastTarget = delayedTarget;
        } else {
        ...
        if (mode.isSelected("Rhomb")) {
            renderRhomb(stack, provider, delayedTarget, alpha);
        } else if (mode.isSelected("Ghost")) {
            renderGhost(stack, provider, delayedTarget, alpha);
        } else if (mode.isSelected("Chain")) {
            renderChain(stack, provider, delayedTarget, alpha, deltaTime);
        } else if (mode.isSelected("Tornado")) {
            renderTornado(stack, provider, delayedTarget, alpha);
        } else if (mode.isSelected("Arrow")) {
            renderArrow(stack, provider, delayedTarget, alpha, deltaTime);
        } else if (mode.isSelected("Inward")) {
            renderInward(stack, provider, delayedTarget, alpha);
        } else if (mode.isSelected("Crystals")) {
            if (crystalList.isEmpty() || lastRenderedTarget != delayedTarget) {
                createCrystals(delayedTarget);
                lastRenderedTarget = delayedTarget;
            }
            renderCrystals(stack, provider, delayedTarget, alpha, deltaTime);
        }
    ...
    private void renderCircle(MatrixStack stack, LivingEntity target, float alpha) {
        ...
        if (hurtProgress > 0) {
            baseColor1 = ColorUtil.lerpColor(baseColor1, 0xFFFF0000, hurtProgress);
            baseColor2 = ColorUtil.lerpColor(baseColor2, 0xFFFF0000, hurtProgress);
        }
    ...
    private void renderChain(MatrixStack stack, VertexConsumerProvider provider, LivingEntity target, float alpha,
        ...
        VertexConsumer consumer = provider
                .getBuffer(ClientPipelines.CHAIN_ESP.apply(Identifier.of("accident", "images/world/chain.png")));
    ...
    private void renderArrow(MatrixStack stack, VertexConsumerProvider provider,
                             LivingEntity target, float alpha, float deltaTime) {
        float dtMs = deltaTime * (1000f / 60f);
        if (!arrowSpinning) {
            arrowIdleTimer += dtMs;
            if (arrowIdleTimer >= ARROW_IDLE_TIME) {
                arrowIdleTimer = 0f;
                arrowSpinning  = true;
                arrowSpinTimer = 0f;
            }
            ...
            arrowSpinTimer += dtMs;
            float progress = Math.min(arrowSpinTimer / ARROW_SPIN_TIME, 1f);
            float eased = (float)(-(Math.cos(Math.PI * progress) - 1.0) / 2.0);
            arrowSpinAngle = eased * 360f;
            if (arrowSpinTimer >= ARROW_SPIN_TIME) {
                arrowSpinAngle = 0f;
                arrowSpinning  = false;
                arrowIdleTimer = 0f;
            }
        ...
        float bob = (float)(Math.sin(System.currentTimeMillis() / 600.0) * 0.07);
        float heightOffset = target.getHeight() + 1.0f + sz * 0.2f;

        stack.push();
        stack.translate(0, heightOffset + bob, 0);
        stack.multiply(net.minecraft.util.math.RotationAxis.POSITIVE_Y.rotationDegrees(arrowSpinAngle));

        VertexConsumer filled = provider.getBuffer(ClientPipelines.CRYSTAL_FILLED);
        drawPyramid(stack, filled, sz,
                withAlpha(color1.getColor(), (int)(220 * alpha)),
                withAlpha(color2.getColor(), (int)(200 * alpha)));

        VertexConsumer glow = provider.getBuffer(ClientPipelines.CRYSTAL_GLOW);
        drawPyramid(stack, glow, sz,
                withAlpha(color1.getColor(), (int)(80 * alpha)),
                withAlpha(color2.getColor(), (int)(70 * alpha)));

        stack.pop();
    }

    private void drawPyramid(MatrixStack stack, VertexConsumer buf,
                             float sz, int topColor, int tipColor) {
        ...
        for (int i = 0; i < 3; i++) {
            double a = Math.toRadians(90 + i * 120.0);
            vx[i] = (float)(Math.cos(a) * r);
            vz[i] = (float)(Math.sin(a) * r);
        }

        for (int i = 0; i < 3; i++) {
            int j = (i + 1) % 3;
            buf.vertex(m, vx[i], yTop, vz[i]).color(topColor);
            buf.vertex(m, vx[j], yTop, vz[j]).color(topColor);
            buf.vertex(m, 0f,    yTip, 0f   ).color(tipColor);
            buf.vertex(m, 0f,    yTip, 0f   ).color(tipColor);
        }

        buf.vertex(m, vx[0], yTop, vz[0]).color(topColor);
        buf.vertex(m, vx[1], yTop, vz[1]).color(topColor);
        buf.vertex(m, vx[2], yTop, vz[2]).color(topColor);
        buf.vertex(m, vx[2], yTop, vz[2]).color(topColor);
    ...
    private void renderRhomb(MatrixStack stack, VertexConsumerProvider provider, LivingEntity target, float alpha) {
        VertexConsumer consumer = provider
                .getBuffer(ClientPipelines.ROMB_ESP.apply(Identifier.of("accident", "images/world/cube.png")));
    ...
    private void renderInward(MatrixStack stack, VertexConsumerProvider provider, LivingEntity target, float alpha) {
        VertexConsumer consumer = provider
                .getBuffer(ClientPipelines.INWARD_ESP.apply(Identifier.of("accident", "images/world/cacto.png")));
    ...
    private void renderTornado(MatrixStack stack, VertexConsumerProvider provider, LivingEntity target, float alpha) {
        VertexConsumer consumer = provider
                .getBuffer(ClientPipelines.TORNADO_ESP.apply(Identifier.of("accident", "images/world/tornado.png")));
    ...
    private void renderGhost(MatrixStack stack, VertexConsumerProvider consumers, LivingEntity target, float alpha) {
        VertexConsumer consumer = consumers
                .getBuffer(ClientPipelines.GHOSTS_ESP.apply(Identifier.of("accident", "images/particle/ghost-glow.png")));
    ...
    private void particle(MatrixStack stack, VertexConsumer consumer, Transformation transformation, float alpha,
        ...
        for (int i = 0; i < 40 * alpha; i++) {
            ...
            int lerpedColor = ColorUtil.lerpColor(baseColor, getNextColor(colorIndex), x);
    ...
    private void renderCrystals(MatrixStack stack, VertexConsumerProvider provider, LivingEntity target, float alpha,
        ...
        if (hurtProgress > 0) {
            baseColor = ColorUtil.lerpColor(baseColor, 0xFFFF0000, hurtProgress);
        }
    ...
    private class Crystal {
        ...
        private void drawFilledCrystal(MatrixStack stack, VertexConsumer consumer, int baseColor, float alphaMultiplier,
            ...
            int darkerColor = withAlpha(ColorUtil.darkenColor(baseColor, 0.7f), finalAlpha);
            int lighterColor = withAlpha(ColorUtil.lightenColor(baseColor, 1.2f), finalAlpha);
        ...
        private void drawBloomEffect(MatrixStack stack, VertexConsumerProvider provider, int baseColor, float anim) {
            VertexConsumer bloomConsumer = provider
                    .getBuffer(ClientPipelines.BLOOM_ESP.apply(Identifier.of("accident", "images/particle/glow.png")));
