...
public class ParticleSpawner {
    ...
    public static void resetParticle(Particle particle, Vec3d playerPos, Vec3d playerVelocity,
                                     double playerSpeed, long lifeTimeMs, Particle.ParticleType type,
                                     boolean physics, float size) {
        double radius = MIN_RADIUS + ThreadLocalRandom.current().nextDouble(MAX_RADIUS - MIN_RADIUS);
        double angle = ThreadLocalRandom.current().nextDouble() * Math.PI * 2.0;
        double spawnX = playerPos.x;
        double spawnZ = playerPos.z;

        if (playerSpeed > 0.05 && playerVelocity.horizontalLengthSquared() > 0.0001) {
            Vec3d normalizedVelocity = playerVelocity.normalize();
            double forwardAngle = Math.atan2(normalizedVelocity.z, normalizedVelocity.x);
            angle = forwardAngle + (ThreadLocalRandom.current().nextDouble() - 0.5) * Math.PI * 0.8;
            double forwardOffset = radius * 0.7 * Math.min(playerSpeed * 8, 1.0);
            spawnX += normalizedVelocity.x * forwardOffset;
            spawnZ += normalizedVelocity.z * forwardOffset;
        }
        ...
        double finalX = spawnX + Math.cos(angle) * radius;
        double finalZ = spawnZ + Math.sin(angle) * radius;
        double finalY = playerPos.y - 5 + ThreadLocalRandom.current().nextDouble() * MAX_HEIGHT;

        double mx = (ThreadLocalRandom.current().nextDouble() - 0.5) * 0.08;
        double my = (ThreadLocalRandom.current().nextDouble() - 0.5) * 0.02;
        double mz = (ThreadLocalRandom.current().nextDouble() - 0.5) * 0.08;

        particle.reset(finalX, finalY, finalZ, mx, my, mz, lifeTimeMs, type, physics, size);
    }
