package accident.util.render.reglass.api;
...
import accident.mixin.GuiRenderStateAccessor;
...
public final class LiquidGlassUniforms {
    ...
    private final GpuBuffer scissorInfo;
    ...
    private LiquidGlassUniforms() {
        ...
        int scissorInfoSize = MAX_WIDGETS * (16 * 4);
        scissorInfo = RenderSystem.getDevice().createBuffer(() -> "reglass ScissorInfo", 130, scissorInfoSize);
    }
    ...
    private void uploadScissorInfo() {
        MinecraftClient mc = MinecraftClient.getInstance();
        int fbH = mc.getFramebuffer().textureHeight;
        float scale = (float) mc.getWindow().getScaleFactor();

        try (var map = RenderSystem.getDevice().createCommandEncoder().mapBuffer(scissorInfo, false, true)) {
            Std140Builder b = Std140Builder.intoBuffer(map.data());

            for (int i = 0; i < MAX_WIDGETS; i++) {
                if (i < widgets.size()) {
                    ScreenRect sc = widgets.get(i).scissorArea();
                    if (sc != null) {
                        float left = sc.getLeft() * scale;
                        float right = sc.getRight() * scale;
                        float top = sc.getTop() * scale;
                        float bottom = sc.getBottom() * scale;

                        b.putVec4(left, fbH - bottom, right, fbH - top);
                    } else {
                        b.putVec4(0f, 0f,
                                (float) mc.getFramebuffer().textureWidth,
                                (float) mc.getFramebuffer().textureHeight);
                    }
                } else {
    ...
    public GpuBufferSlice getWidgetInfoSlice() {
        return new GpuBufferSlice(this.widgetInfo, 0, this.widgetInfo.size());
    }
    ...
    public GpuBuffer getScissorInfoBuffer() { return scissorInfo; }
