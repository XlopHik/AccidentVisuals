...
public final class LiquidGlassPrecomputeRuntime {
    ...
    private void ensureTempTarget(int w, int h) {
        if (blurTempTex == null || blurTempTex.getWidth(0) != w || blurTempTex.getHeight(0) != h) {
            ...
            blurTempTex = device.createTexture("reglass blurTemp",
                    GpuTexture.USAGE_RENDER_ATTACHMENT | GpuTexture.USAGE_TEXTURE_BINDING,
                    TextureFormat.RGBA8, w, h, 1, 1);
            blurTempView = device.createTextureView(blurTempTex);
        }
    }
    ...
    private void ensureOutputForRadius(int w, int h, int radius) {
        ...
        if (tex == null || tex.getWidth(0) != w || tex.getHeight(0) != h) {
            ...
            GpuDevice device = RenderSystem.getDevice();
            GpuTexture newTex = device.createTexture("reglass blurred r=" + radius,
                    GpuTexture.USAGE_RENDER_ATTACHMENT | GpuTexture.USAGE_TEXTURE_BINDING,
                    TextureFormat.RGBA8, w, h, 1, 1);
            GpuTextureView newView = device.createTextureView(newTex);
    ...
    public void run() {
        ...
        ByteBuffer vertexData = ByteBuffer.allocate(vertices.length * 4);
        vertexData.asFloatBuffer().put(vertices);
        var quadVB = RenderSystem.getDevice().createBuffer(() -> "quad", 64, vertexData);
        ...
        for (int k = 0; k < max; k++) {
            ...
            try (RenderPass pass = ce.createRenderPass(() -> "reglass blur X r=" + radius, blurTempView, OptionalInt.empty())) {
                ...
                pass.bindTexture("DiffuseSampler", main.getColorAttachmentView(), sampler);
            ...
            try (RenderPass pass = ce.createRenderPass(() -> "reglass blur Y r=" + radius, blurredViewByRadius.get(radius), OptionalInt.empty())) {
                ...
                pass.bindTexture("DiffuseSampler", blurTempView, sampler);
