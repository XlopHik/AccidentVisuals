...
public class MaskDiffPipeline {
    ...
    private static final Identifier PIPELINE_ID = Identifier.of("accident", "pipeline/mask_diff");
    private static final Identifier VERTEX_SHADER = Identifier.of("accident", "core/mask_diff");
    private static final Identifier FRAGMENT_SHADER = Identifier.of("accident", "core/mask_diff");
    ...
    /** Vanilla's projection near plane; stable across every MC version that matters here. */
    ...
    private void prepareUniformData(int width, int height) {
        float far = net.minecraft.client.MinecraftClient.getInstance().gameRenderer.getFarPlaneDistance();

        dataBuffer.clear();
        dataBuffer.putFloat(width);
        dataBuffer.putFloat(height);
        dataBuffer.putFloat(NEAR_PLANE);
        dataBuffer.putFloat(far);
        dataBuffer.flip();
    }
