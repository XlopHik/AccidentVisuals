...
public class GlassCompositePipeline {
    ...
    private static final Identifier PIPELINE_ID = Identifier.of("accident", "pipeline/glass_composite");
    private static final Identifier VERTEX_SHADER = Identifier.of("accident", "core/glass_composite");
    private static final Identifier FRAGMENT_SHADER = Identifier.of("accident", "core/glass_composite");

    // альфа-блендинг, не замена: вне маски шейдер отдаёт alpha 0, см. glass_composite.fsh
    ...
    public void composite(GpuTextureView targetView, GpuTextureView sceneView,
                          ...
                          float edgeGlowIntensity, float time, float waveIntensity, int style) {

        ensureInitialized();
        prepareUniformData(width, height, saturation, reflect, tintColor, tintIntensity, edgeGlowIntensity, time, waveIntensity, style);
    ...
    private void prepareUniformData(int width, int height, float saturation,
        ...
        dataBuffer.putFloat(time);        // settings.z — было 0
        dataBuffer.putFloat(waveIntensity); // settings.w — было 0
