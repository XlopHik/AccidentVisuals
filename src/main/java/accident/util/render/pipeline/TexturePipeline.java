...
public class TexturePipeline {
    ...
    private static final Identifier PIPELINE_ID = Identifier.of("accident", "pipeline/texture");
    private static final Identifier VERTEX_SHADER = Identifier.of("accident", "core/texture");
    private static final Identifier FRAGMENT_SHADER = Identifier.of("accident", "core/texture");
    ...
    private void uploadAndDraw(MinecraftClient client, int textureGlId) {
        ...
        accident.util.render.DrawTimes.texture(accidentT0);

        GL11.glBindTexture(GL11.GL_TEXTURE_2D, 0);
    }

    // в каком поле GpuTexture лежит GL-хэндл, ищем через рефлексию один раз на класс и кэшируем -
    // сам поиск дорогой, а раньше он делался на каждой отрисовке текстуры
    // Optional нужен, чтобы класс без нужного поля не пере-проверялся каждый раз заново
    private static final Map<Class<?>, Optional<Field>> GL_ID_FIELDS = new ConcurrentHashMap<>();

    private int getTextureGlId(GpuTexture gpuTexture) {
        Optional<Field> cached = GL_ID_FIELDS.get(gpuTexture.getClass());
        if (cached == null) {
            cached = resolveGlIdField(gpuTexture);
            // промах не кэшируем - текстура могла попасться до окончания загрузки
            if (cached.isPresent()) {
                GL_ID_FIELDS.put(gpuTexture.getClass(), cached);
            }
        }
        if (cached.isEmpty()) {
            return 0;
        ...
        try {
            return cached.get().getInt(gpuTexture);
        } catch (Exception e) {
            return 0;
        }
    }

    private static Optional<Field> resolveGlIdField(GpuTexture instance) {
        ...
        // запасной вариант: берём первое int-поле с живым хэндлом, кэшируется само поле, а не значение
        try {
            for (Field f : type.getDeclaredFields()) {
                if (f.getType() != int.class) continue;
                f.setAccessible(true);
                if (f.getInt(instance) > 0) {
