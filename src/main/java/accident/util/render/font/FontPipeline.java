...
public class FontPipeline {
    ...
    private static final Identifier PIPELINE_ID = Identifier.of("accident", "pipeline/msdf");
    private static final Identifier SHADER_ID = Identifier.of("accident", "core/msdf");
    ...
    // раньше view атласа создавался и уничтожался на каждый flush - атласов мало и они живут долго,
    // поэтому теперь view просто кэшируется вместе с текстурой (ключ - сама текстура)
    private final Map<GpuTexture, GpuTextureView> atlasViews = new IdentityHashMap<>();
    ...
    private float currentGlowWidth = 0;
    ...
    public void drawText(FontAtlas atlas, String text, float x, float y, float size, int color,
        ...
        drawText(atlas, text, x, y, size, color, outlineWidth, outlineColor, rotation, 0f);
    }

    public void drawText(FontAtlas atlas, String text, float x, float y, float size, int color,
                         float outlineWidth, int outlineColor, float rotation, float glowWidth) {
        ...
        if (currentAtlas != null && (currentAtlas != atlas || currentOutlineWidth != outlineWidth
                || currentOutlineColor != outlineColor || currentGlowWidth != glowWidth)) {
            flush();
        }
    ...
    public void flush() {
        ...
        prepareUniformData(client, currentAtlas, currentOutlineWidth, currentOutlineColor, currentGlowWidth);

        int size = dataBuffer.remaining();
        if (uniformBuffer == null || uniformBuffer.size() < size) {
            if (uniformBuffer != null) uniformBuffer.close();
            uniformBuffer = RenderSystem.getDevice().createBuffer(
                    () -> "accident:font_uniform",
        ...
        GpuTextureView textureView = atlasViews.computeIfAbsent(gpuTexture,
        ...
        accident.util.render.DrawTimes.text(accidentT0);

        charBatch.clear();
        currentAtlas = null;
    }

    private void prepareUniformData(MinecraftClient client, FontAtlas atlas, float outlineWidth, int outlineColor, float glowWidth) {
        dataBuffer.clear();
        ...
        dataBuffer.putInt(Math.round(glowWidth * 1000f));
        dataBuffer.putInt(0);
        dataBuffer.putInt(0);
    ...
    public void close() {
        ...
        for (GpuTextureView view : atlasViews.values()) {
            view.close();
        }
        atlasViews.clear();
        initialized = false;
