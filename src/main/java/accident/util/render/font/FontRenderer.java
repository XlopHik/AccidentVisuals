...
public class FontRenderer {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger("accident/FontRenderer");
    ...
    public void loadFont(String name, String path) {
        Identifier jsonId = Identifier.of("accident", "fonts/" + path + ".json");
        Identifier textureId = Identifier.of("accident", "fonts/" + path + ".png");
    ...
    // мягкое свечение, затухающее от края буквы - в отличие от outline выше,
    // который просто чёткое кольцо фиксированного радиуса
    public void drawTextWithGlow(String fontName, String text, float x, float y, float size,
                                 int color, int glowColor, float glowWidth) {
        FontAtlas atlas = fonts.get(fontName);
        if (atlas == null) return;
        pipeline.drawText(atlas, text, x, y, size, color, 0f, glowColor, 0f, glowWidth);
    }
