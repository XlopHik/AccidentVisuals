...
public class Render3D implements IMinecraft {
    ...
    public final List<GradientQuad> GRADIENT_QUAD_TRAIL = new ArrayList<>();
    ...
    public final List<Quad> QUAD_ESP = new ArrayList<>();
    public final List<Line> LINE_ESP = new ArrayList<>();
    ...
    public void onWorldRender(WorldRenderEvent e) {
        ...
        renderTrailQuads(matrices, immediate, cameraPos);
        renderQuads(matrices, immediate, cameraPos);
        renderLines(matrices, immediate, cameraPos);
        renderEspGeometry(matrices, immediate, cameraPos);

        immediate.draw();
    }

    private void renderEspGeometry(MatrixStack matrices, VertexConsumerProvider.Immediate immediate, Vec3d cameraPos) {
        if (QUAD_ESP.isEmpty() && LINE_ESP.isEmpty()) return;

        if (!QUAD_ESP.isEmpty()) {
            VertexConsumer buffer = immediate.getBuffer(ClientPipelines.ESP_BLOCK_LAYER);
            for (Quad quad : QUAD_ESP) {
                drawQuadVertex(matrices, buffer, quad, cameraPos);
            }
            QUAD_ESP.clear();
            immediate.draw(ClientPipelines.ESP_BLOCK_LAYER);
        }
    ...
    public void drawBoxEsp(Box box, int color, float width) {
        ...
        drawQuadEsp(new Vec3d(x1,y1,z1), new Vec3d(x2,y1,z1), new Vec3d(x2,y1,z2), new Vec3d(x1,y1,z2), fillColor);
        drawQuadEsp(new Vec3d(x1,y1,z1), new Vec3d(x1,y2,z1), new Vec3d(x2,y2,z1), new Vec3d(x2,y1,z1), fillColor);
        drawQuadEsp(new Vec3d(x2,y1,z1), new Vec3d(x2,y2,z1), new Vec3d(x2,y2,z2), new Vec3d(x2,y1,z2), fillColor);
        drawQuadEsp(new Vec3d(x1,y1,z2), new Vec3d(x2,y1,z2), new Vec3d(x2,y2,z2), new Vec3d(x1,y2,z2), fillColor);
        drawQuadEsp(new Vec3d(x1,y1,z1), new Vec3d(x1,y1,z2), new Vec3d(x1,y2,z2), new Vec3d(x1,y2,z1), fillColor);
        drawQuadEsp(new Vec3d(x1,y2,z1), new Vec3d(x1,y2,z2), new Vec3d(x2,y2,z2), new Vec3d(x2,y2,z1), fillColor);
    }

    public void drawQuadEsp(Vec3d x, Vec3d y, Vec3d w, Vec3d z, int color) {
        QUAD_ESP.add(new Quad(null, x, y, w, z, color));
    }
    ...
    private void renderTrailQuads(MatrixStack matrices, VertexConsumerProvider.Immediate immediate, Vec3d cameraPos) {
        if (GRADIENT_QUAD_TRAIL.isEmpty()) return;

        VertexConsumer buffer = immediate.getBuffer(ClientPipelines.TRAIL_LAYER);
        for (GradientQuad quad : GRADIENT_QUAD_TRAIL) {
            drawGradientQuadVertex(matrices, buffer, quad, cameraPos);
        }
        GRADIENT_QUAD_TRAIL.clear();
        immediate.draw(ClientPipelines.TRAIL_LAYER);
    }

    public void drawTrailQuad(Vec3d p1, Vec3d p2, Vec3d p3, Vec3d p4, int c1, int c2, int c3, int c4) {
        GRADIENT_QUAD_TRAIL.add(new GradientQuad(p1, p2, p3, p4, c1, c2, c3, c4));
    }
