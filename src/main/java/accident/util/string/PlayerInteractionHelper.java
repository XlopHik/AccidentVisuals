...
public class PlayerInteractionHelper implements IMinecraft {
    ...
    public void interactItem(Hand hand, Angle angle) {
        try {
            boolean isDev = net.fabricmc.loader.api.FabricLoader.getInstance().isDevelopmentEnvironment();

            String methodName = isDev ? "syncSelectedSlot" : "method_2911";

            java.lang.reflect.Method syncMethod = mc.interactionManager.getClass().getDeclaredMethod(methodName);

            syncMethod.setAccessible(true);
            syncMethod.invoke(mc.interactionManager);
